...
class HiveFlowMCPServer {
  ...
  private hiveflowClient: AxiosInstance;
  private config: HiveFlowConfig;

  constructor(config: HiveFlowConfig) {
    this.config = config;
...
program
  .option('--api-url <url>', 'HiveFlow API URL', process.env.HIVEFLOW_API_URL || 'https://api.hiveflow.com')
